module WebfaceComponentHelper
  ...
  def component(name, attrs={}, &block)
    render partial: "components/#{name}", locals: { options: attrs.delete(:options) || {}, attrs: attrs, block: block }
  end
  ...
  def button_link(caption, path, options={})
    component_block :button, "a", { class: "button", href: path }.merge(options) do
      caption
    end
  end

  def post_button_link(caption, path, verb, options={})
    render partial: "components/post_button_link", locals: { caption: caption, path: path, verb: verb, options: options }
  end
